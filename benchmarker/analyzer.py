# Do not use this file, instead use analyzer.ipynb
...
def pretty_print_data(dfs, size):
    table = Table(
        title=f"Size {size} | n log n: {round(size * math.log(size, 2), 4)} | 1.5 n log n: {round(size * 1.5 * math.log(size, 2), 4)}"
    )
    table.add_column("")
    ...
    table.add_row(
        "Comparisons",
        str(dfs[size]["sorted"]["comparison_count"].mean()),
        str(dfs[size]["reverse"]["comparison_count"].mean()),
        str(dfs[size]["shuffle"]["comparison_count"].mean()),
    )
    table.add_row(
        "Time",
        str(dfs[size]["sorted"]["time"].mean()),
        str(dfs[size]["reverse"]["time"].mean()),
        str(dfs[size]["shuffle"]["time"].mean()),
    )
            ...
            tempdf = pd.DataFrame(
                {
                    "Size": size,
                    "Type": manip_type,
                    "Time": dfs[size][manip_type]["time"].mean(),
                    "Comparisons": dfs[size][manip_type]["comparison_count"].mean(),
                },
                index=[0],
            )
